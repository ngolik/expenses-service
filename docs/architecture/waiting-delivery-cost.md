...
- Happy path (create): handler binds JSON → `service.AddWaitingDeliveryCost` checks delivery id / user id / amount are non-zero → calls `UserValidator.UserExists(userID)` → on `true`, persists via `ExpenseRepository.Create` → 200.
...
| Auth-service base path | Trust the spec's confirmed `/api/users/{id}` (no `/auth` prefix) vs. `auth-service`'s own `application.yml`, which sets `server.servlet.context-path: /auth/` | Trust the spec's confirmed route; make the base URL fully configurable via `AUTH_SERVICE_BASE_URL` | The cross-repo contract explicitly confirms `/api/users/{id}` as the direct (non-gateway) route and this repo does not own `auth-service`'s config. `[INFERRED — please validate]` — `application.yml`/`application-dev.yml` in `auth-service` both set a `/auth/` context-path, which would normally prefix every route including this one. If that context-path is in effect for the environment this actually gets deployed to, `AUTH_SERVICE_BASE_URL` needs to be set to `http://<host>:<port>/auth` (or the client's path construction revisited) — this needs to be re-verified against a running `auth-service` instance, not just its controller source, before go-live. |
